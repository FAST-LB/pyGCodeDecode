...
class planner_block:
    def calc_JD(self,vel_0:velocity,vel_next:velocity,p_settings:state.p_settings):
        ...
        if p_settings.jerk == 0: return 0
        JD_delta    = 0.4 * p_settings.jerk*p_settings.jerk / JD_acc #[2]
        JD_minAngle = 18
        JD_maxAngle = 180-18
        vel_0_vec   = vel_0.get_vec()
        vel_1_vec   = vel_next.get_vec()
        if vel_0.get_abs() == 0 or vel_next.get_abs() == 0: return 0
        ###calculate junction angle 
        JD_cos_theta = (np.dot(-np.asarray(vel_0_vec),np.asarray(vel_1_vec))/(np.linalg.norm(vel_0_vec)*np.linalg.norm(vel_1_vec))) #cos of theta, theta: small angle between velocity vectors
    ...
    def move_maker2(self,v_end):
        ...
        def trapez(extrusion_only=False):
            ...
            if pos_end.is_travel(pos_begin) or pos_end.is_extruding(pos_begin): self.segments.append(segment_A)
            ### B --
            travel_const= (distance-travel_ramp_down-travel_ramp_up)
            v_const     = vel_const.get_abs() #abs of vel const
            ...
            if pos_end.is_travel(pos_begin) or pos_end.is_extruding(pos_begin): self.segments.append(segment_B)
            ### C \
            t2          = segment_B.t_end
            ...
            if pos_end.is_travel(pos_begin) or pos_end.is_extruding(pos_begin): self.segments.append(segment_C)
    ...
    def self_correction(self,tolerance=float("1e-12")):
        ...
        if not self.next_blck is None:
            ...
            if not same_vel:
                error_vel  = self.get_segments()[-1].vel_end.get_abs() - self.next_blck.get_segments()[0].vel_begin.get_abs()
